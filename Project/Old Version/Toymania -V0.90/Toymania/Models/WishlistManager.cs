...
namespace Toymania.Models
{
    public class WishlistManager
    {
        ...
        public int LastRecordW()
        {
            ...
            {
                IQueryable<int> WILIQ = db.Wishlist.Select(x => x.WishlistId);
        ...
        public bool ToyDontExist(Toy t, string i)
        {
            //var a = from B in db.Wishlist where B.ToysId == t.ToysId && B.Email == w.Email select B;
            var I = i;
            
            var a = (from B in db.Wishlist where B.ToysId == t.ToysId && B.Email == i select B).ToList().FirstOrDefault();
            var b = a.ToysId;
            try
            {
                
                //var c = a.First().Email;
                if (b != t.ToysId)                      //als het niet bestaat dan returnt dit een error dus try-catch implementeren om de error te handelen
                {
                    return true;
                }
                else
